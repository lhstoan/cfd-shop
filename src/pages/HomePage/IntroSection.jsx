...
import { getImage } from './useHomePage';
...
const IntroSection = ({ introProducts }) => {
	...
	useEffect(() => {
		...
		//event "load" affects performance
					...
					<div className="col-lg-4">
						<div className="intro-banners">
							{introProducts?.map((product, index) => {
								...
								return (
									<div className="banner mb-lg-1 mb-xl-2" key={product.id || index}>
										<Link to={productPath}>
										...
										<div className="banner-content">
											<h3 className="banner-title">
												<Link to={productPath} style={windowWidth > 1024 ? responsiveStyle : null}>{name}</Link>
											</h3>
											<Link to={productPath} className="banner-link">Shop Now <i className="icon-long-arrow-right" /></Link>
										</div>
									</div>
								);
							})}
